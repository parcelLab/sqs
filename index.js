...
module.exports = function (options) {
  ...
  var awsDefaults = {}
  if (options.access) awsDefaults.key = options.access
  if (options.secret) awsDefaults.secret = options.secret
  if (options.access && options.secret) awsDefaults.sign_version = 4
    ...
    request(queryURL('CreateQueue', '/', { QueueName: name }), function (err) {
      ...
      request(queryURL('GetQueueUrl', '/', { QueueName: name }), function (err, res) {
        if (err || res.statusCode !== 200) {
          ...
          return onresult(err || new Error('invalid status code from GetQueueUrl: ' + res.statusCode + (errMessage ? '. ' + errMessage : '')))
        }
